...
from . import softmax_attention_reach  # noqa: F401  (multi-input via reach.py)
...
def _reach_layer_star(layer: nn.Module, input_sets: List, method: str, **kwargs) -> List:
    ...
    # ----- ViT activations / norms (this PR) -----
    elif isinstance(layer, nn.GELU):
        # Branch on approximate=tanh vs erf: the tanh dip falls below the
        # erf floor, so erf-form routing is unsound for GELU(approximate='tanh').
    ...
    # ----- ViT tokens / attention / patches (this PR) -----
    elif isinstance(layer, _SoftmaxAttention):
        raise NotImplementedError(
            "SoftmaxAttention requires multi-input (Q, K, V) dispatch "
            "via n2v.nn.reach."
        )
